        ...
        {features.map(([num, title, text]) => (
          <div className="col-md-4" key={num}>
            <article className="feature-card">
              ...
              <h2>{title}</h2>
              <p>{text}</p>
              <NavLink to="/contact">Talk to our team →</NavLink>
            </article>
          </div>
        ))}
      </div>
...
function Contact() {
  ...
  const submitForm = async (event) => {
    ...
    setState({ sending: true, message: "", error: false });
    try {
      ...
      setState({ sending: false, message: result.message, error: false });
    } catch (error) {
      setState({ sending: false, message: error.message, error: true });
    }
  };
  return (
    <main className="inner-page container py-5">
      <span className="eyebrow">Contact Nexus</span>
      <h1>Let’s build something useful.</h1>
      <p className="page-intro">
        Tell us about the work you want to transform. Your message is securely
        stored in our customer-lead system.
      </p>
      <form className="contact-form mt-4" onSubmit={submitForm}>
        ...
        {state.message && (
          <p
            className={`form-message ${state.error ? "form-error" : ""}`}
            role="status"
          >
            {state.message}
          </p>
      ...
      <nav className="navbar navbar-expand-md navbar-dark container">
        ...
        <div className="collapse navbar-collapse" id="siteNav">
          <div className="navbar-nav ms-auto align-items-md-center">
            ...
            <NavLink className="nav-contact ms-md-3" to="/contact">
